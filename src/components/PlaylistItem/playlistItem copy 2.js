...
function PlaylistItem(props) {
  ...
  const handleClickLike = async () => {
    ...
    try {
      if (isLiked) {
        // Выполните запрос для удаления трека из избранного
        const response = await delToFavorites({ accessToken: user.token.access, trackId: props.track.id });
        console.log(response);
      } else {
        // Выполните запрос для добавления трека в избранное
